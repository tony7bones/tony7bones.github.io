...
`addons/hosted/<id>/` holds mirrored third-party-repo trees (static, hand-committed metadata; not zipped or indexed by the generator). **Six of the `hosted/<id>/` entries are OUR OWN add-ons**, with source in a sibling repo. Two are metadata-only mirrors whose zip comes from a GitHub Release, and four carry the zip here (the count read four until 2026-08-27; `plugin.video.estuary8.search` had been hosted since 2026-08-03 without being listed, and `skin.estuary.pov` was added on the 27th):

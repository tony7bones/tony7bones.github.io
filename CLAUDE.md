...
> - `docs/playbooks/mac-mini-media-server.md` - **the `mini` box that serves every Kodi client** (NFS `~/Kodi/Share` + `~/Kodi/Backup` on the internal SSD; the T7 is SMB-only spare). The always-on server fix (`pmset sleep 0`, it was sleeping after 1 idle min and dropping all shares on logout), boot-without-login (FileVault off + `autorestart`), passwordless-sudo setup, and when to promote the T7 to an NFS media drive.

...
> - `docs/playbooks/mac-mini-media-server.md` - **the `Mini` box that serves every Kodi client** (serves `KodiShare`/`KodiBackup`/`Public` over BOTH NFS for the Kodi boxes and SMB for the Macs, same folders both ways; home is SMB-only; the T7 external drive is unplugged/set aside). The always-on fix (`pmset sleep 0`), boot-without-login (FileVault off + `autorestart`), passwordless sudo, and the hard-won macOS rules: NFS is for appliances and SMB is for Macs (NFS never lists shares in Finder and leaves ghost sessions), SMB guest is dead on macOS 26, never NFS-export the home dir, external volumes need daemon Full Disk Access.

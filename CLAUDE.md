...
Besides the proxy, the repo ships two first-party Setup/library add-ons plus the Estuary MOD V2+ skin patch — **four** first-party add-ons in total (counting the proxy). Current shipped versions on `main`: `repository.tony7bones` 2.2.1, `script.tony7bones.bootstrap` 1.4.0, `script.tony7bones.modv2plus` 1.4.7, `script.module.tony7bones` 1.1.3.
...
- `_configure_box()` — weather + interface prefs, now driven by the **per-device `.env`** (see below): provider → `weather.multi`, RSS ticker on, top-bar weather (`Skin.SetBool(show_weatherinfo)`), then `_copy_device_files()` + `_ensure_iptv_custom_tv_groups(box_env)`. It first calls `read_box_env` (reading the per-device `tony7bones.env` the provisioner pushed to the box), then applies it via `_apply_weather_from_env` / `_ensure_iptv_custom_tv_groups(box_env)` / `_apply_rss_from_env`, and **read-then-removes** the env file so no secret lingers. Weather locations (up to 5), the Weatherbit/OWM keys, IPTV custom groups + m3u/EPG, and RSS feeds all come from the env — the location is **env-driven, NOT a hardcoded Sacramento constant** (note `loc1_url` is the field weather.multi actually fetches by, NOT lat/lon). (On the `modular-setup` branch the IPTV half of this lives in the shared library's `tony7bones.setup.iptv` layer — see "IPTV is two halves" below — and `_configure_box`'s legacy IPTV slot runs inside the same PVR-disabled config window.)

  **Per-device `.env` model.** One gitignored `.env.<device>` per box (in the repo) is the source of truth for that box's weather, IPTV, RSS, and device name/web/settings-level. The provisioner derives a per-device `tony7bones.env` and pushes it to `BOX_ENV_PATH=/storage/emulated/0/kodi/tony.7.bones/tony7bones.env`; the bootstrap reads it in `_configure_box`, applies it, then deletes it. The committed `.env.device.example` is the **only** tracked `.env*` — a placeholder template (`.gitignore` un-ignores it, and `test_secret_leak.py` allowlists it via `_EXAMPLE_ENVS`).
...
> - `docs/plans/modular-setup.md` — the **LIVE** plan for the modular "0-1-2" Setup rewrite (branch `modular-setup`): design + phase log + the next-phase prep. Read it first when working on that branch.

...
Kodi's own tvOS source. The review also surfaced the _correct_ fix. See the verdict at the
...
**Root cause (from Kodi Omega source, `xbmc@f8815ee4`):** Kodi's tvOS home is under
`Library/Caches`, which the system may purge, so Kodi vectors `userdata/*.xml` into the app's
**NSUserDefaults** (a binary plist in a non-purged domain). It does **NOT** rewrite
`userdata/guisettings.xml` from that store on launch - reads check the **key first** and fall
back to POSIX, so a key **SHADOWS** the disk file (`TVOSFile.cpp:70-122`). The old wording here
("rewrites from that mirror on launch") came from the Kodi Wiki, is FALSE, and is the belief that
led to deleting a disk copy that nothing would ever restore.
...
Stop trying to make a single post-restore write _persist_. Instead **re-apply the restored
...
- **A1 - The stash + marker files survive an unclean quit + reboot on tvOS.** The whole
  design reads from them on the _second and later_ boots. If tvOS's ~500 KB limit / the
...
- **A3 - Re-applying every boot does not fight the user.** If self-terminate (D3) never
  ...
  reverted on the next boot. This could be _worse_ than the original bug.
...
All three reviewers were told to _prove it cannot work_. They agreed, by different routes:
...
_base-backup_ values every boot is a pure regression (reverts the user's changes every
...
N-boot bound stops re-asserting while NSUserDefaults is _still_ stale → **the original bug
...
_shadowed_, not "un-flushed."** `apply_guisettings` (live store) is lost on the unclean quit,
...
2. Keep the shipped `apply_guisettings` JSON-RPC so a _clean_ shutdown path also holds.
...
entire list incl. TV/IPTV** (instance settings are `.xml` and vector too). This is _simpler
and more complete_ than the rejected design.
...
- **Write-through-VFS is verified in Kodi SOURCE, not yet on hardware.** The Xcode-CLI plist
  ...
  before shipping. (This mechanism _is_ confirmable in source today, unlike the rejected

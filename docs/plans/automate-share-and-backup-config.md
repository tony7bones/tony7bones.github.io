...
**EZ Maintenance++ install + configure** (`script.ezmaintenanceplusplus`, our own
fork with the NFS/SMB retry hardening - source now at `moquette/ezmaintenanceplusplus`,
its own repo since 2026-07-14; see Decision C, RESOLVED). Owner
...
- **B. Per-device backup subfolder.** RESOLVED: yes, `Kodi/Backup/<device-slug>` (no
  ...
  `DEVICE_NAME`.
- **C. WHICH EZ Maintenance add-on.** RESOLVED: standardize on our own `++` fork
  (`script.ezmaintenanceplusplus`, source at `moquette/ezmaintenanceplusplus`); drop
  peno64's `script.ezmaintenanceplus` from the

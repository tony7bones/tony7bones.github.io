...
>
> **CORRECTION (2026-07-14, repo migration + box identity).** Phase 4's release steps below ("bump `addons/script.ezmaintenanceplusplus/addon.xml`... commit + push `main`") describe a mechanism that no longer exists - EZ Maintenance++'s source moved to its own repo, `moquette/ezmaintenanceplusplus`; see `~/Code/moquette/kodi/.claude/skills/ezm-backup-doctor/SKILL.md` for the current release procedure. Separately, this doc's `atv-1 = 192.168.7.220` was later found WRONG (`.220` answers ping but is not the box): the real box is `192.168.7.183`, whose Kodi friendlyname now reports `atv2` - see `docs/incident-2026-07-14-ezmpp-restore-wiped-custom-menu-tvos.md`. Re-identify by JSON-RPC `System.FriendlyName`, never a remembered IP.

...
> **The read-plain / write-through split, applied again (restore, tvOS).** EZM's
> `nsud.rewrite_userdata_xml` (the tvOS restore durability rewrite, 2026.07.08.2)
> obeys this playbook's rule in the read direction and inverts it in the write:
> it reads each restored `userdata/*.xml` with plain `open()` (never `xbmcvfs`,
> because this add-on wrote that file with plain `zipfile` on extract - exactly
> the local-read trap above), then WRITES it back THROUGH `xbmcvfs` in one call
> so `CTVOSFile` vectors it into NSUserDefaults (the only durable tvOS store).
> The write must go through `xbmcvfs` on purpose; only the read is forced to be
> plain. That rewrite later grew a duplicate-File-Manager-entry bug and a
> read-back-gated POSIX-drop fix - see `docs/playbooks/ezm-restore-hardening.md`.
>
...
> different mechanism.
>
> **The texture-loader / ControlImage face (2026.07.13, EZM 2026.07.13.6).** The SAME
> read-empty-on-tvOS bug hits `xbmcgui.ControlImage`: the Dropbox sign-in QR
> (`dropbox_remote._qr_image`) was a valid PNG written with plain `open()`, so on Apple TV
> the texture loader (which reads via Kodi's VFS) got nothing and drew a blank/broken-image
>
> - while Fire TV (Android, no sandbox restriction) rendered it fine. Fix: write the image
>   THROUGH `xbmcvfs.File(path,"w")` and hand ControlImage the `special://` path. Any add-on
>   that generates an image and displays it must obey the write-through rule. Full writeup:
>   `docs/incident-2026-07-13-ezmpp-dropbox-qr-blank-tvos.md` (also covers Kodi 21.3 refusing
>   grayscale PNGs and the per-path texture cache that made a first failed load stick).

...
## 2.4 Kodi add-on settings can fail SILENTLY. Do not store truth in them.

`xbmcaddon.Addon(id).setSettingBool(key, value)` is a **silent no-op** when `key`
is not in the add-on's registered settings schema. It does not raise. It does not
write. It returns as if it worked. This happens on any box that installed the
add-on BEFORE it shipped a `resources/settings.xml`, because Kodi caches what
settings an add-on has.

Found on the Apple TV while building the MOD V2++ master toggle (1.6.0): the
toggle restored the skin perfectly, logged nothing, and lost the user's choice.
On the next boot the service read the default (`enabled`) and would have switched
the box back on. A toggle that lies.

The rule that follows: **anything the boot service must obey is stored as a plain
file we write ourselves**, in `special://profile/addon_data/<addon-id>/`. One
`open().write()`. No schema, no cache, no shutdown to depend on. Compare the two
mechanisms we do NOT use for durable state:

| Mechanism                            | Fails how                                                         |
| ------------------------------------ | ----------------------------------------------------------------- |
| `Skin.SetBool` / skin `settings.xml` | only reaches disk on a CLEAN shutdown; a force-quit loses it      |
| `Addon.setSettingBool`               | silent no-op when the schema is not registered; no error, no file |
| plain file in `addon_data`           | writes immediately, survives a kill, readable by other add-ons    |

A skin bool is still the right tool for a control's `<selected>`, because XML
cannot read a file. But it is a DISPLAY MIRROR of the truth, never the truth.

## 2.45 A permanent tab, and the wedge it creates

MOD V2++ 1.6.0 folds Apply/Restore into one master switch that lives in the Skin
Settings category the add-on itself installs. Two consequences worth knowing:

- **The shell must survive its own off state.** `SkinSettings.xml` (the shell) is
  deployed in BOTH states and excluded from `restore_patches`; only the other 7
  PATCH_FILES revert. If the shell reverted, switching off would delete the only
  control that can switch back on. Its own nav column renders thin or bold via two
  visibility-gated labels, because a `<font>` binds at window load but `<visible>`
  is re-evaluated every render.
- **THE WEDGE.** A MOD V2 skin update overwrites every skin XML with stock. On a
  switched-OFF box that takes the tab with it, and the boot service is standing
  down (intent says off), so nothing brings it back: no tab, no control, no way to
  ever re-enable. The service therefore re-lays the SHELL ALONE whenever it is
  missing or stale (`lay_shell`), and never the tweaks. Non-destructive, and it
  cannot silently switch a box back on.
- The category item is gated on `System.AddonIsEnabled(...)`, not `System.HasAddon(...)`.
  `HasAddon` stays true for a DISABLED add-on, whose `RunScript` no-ops, so the tab
  would sit there inert. Kodi runs no code on uninstall, so this condition is the
  only cleanup we get. Hidden static `<item>`s are excluded from list layout, so no
  gap is left behind (verified on hardware).

## 2.45 ReloadSkin, and the four traps around returning the user to a control

Shipping the MOD V2++ master toggle (1.6.2) burned an evening on these. All four
were found on hardware; none is visible from the source.

- **`ReloadSkin()` does NOT dump the user on the home screen.** It rebuilds the
  window they are standing in and resets its lists. In Skin Settings that means
  the category list snaps back to item 0, whose label is `$LOCALIZE[31203]`:
  **"Home menu"**. Landing eleven rows above your control on a tab called "Home
  menu" reads exactly like being thrown to the home screen. `GUI.GetProperties`
  still reports `currentwindow: 10035` throughout, and `Home.xml` never loads.
- **`xbmc.executebuiltin("ReloadSkin()", True)` does not wait for the reload.**
  The `wait` flag waits for the builtin to be DISPATCHED. Measured: a return
  sequence ran to completion in 113ms, acting on the window Kodi was about to tear
  down. Never sleep or trust `wait=True` here; wait for a positive signal.
- **`Control.HasFocus(<id>)` LIES after a rebuild.** Kodi keeps reporting the old
  focused control id even though the list has reset and the panel holding that
  control is hidden. A GUI probe on a Fire TV showed
  `control=1102 Control.HasFocus(1102)=True` while `category='Home menu'` and
  `Container(9000).HasFocus(11)=False`. **`Container(...).HasFocus(<item id>)` is
  honest.** Trust the container, never the control id.
- **The skin reloads TWICE.** Ours, then `script.skinshortcuts` when it finishes
  rebuilding the home menu, which Skin Settings' own `<onunload>` fires. A one-shot
  return lands between the two and is silently undone. Re-assert the focus in a
  loop until the selection HOLDS (3s settle), then focus the control.

The working shape, in `default.py::_return_to_master_toggle`: wait for
`!Container(9000).HasFocus(11)` (proof the rebuild happened), then loop
re-selecting the category until it sticks, then `SetFocus(<control>)`. Every wait
polls, every wait has a timeout, and giving up leaves the user where the reload
left him, which is the status quo.

## 2.46 Instrument the Fire TV instead of asking a human

tvOS cannot screenshot (`TakeScreenshot` crashes Kodi), so ATV1 work costs the
owner a manual look per iteration. The Fire TVs can, and a GUI probe beats pixels:
push a dev-only script add-on whose `probe` argument logs
`System.CurrentControlId`, `System.CurrentWindow`,
`Container(9000).ListItem.Label`, and any `getCondVisibility` you care about, then
drive with `Input.Select` / `Input.Left` over JSON-RPC and read the trail out of
`kodi.log`. `adb screencap` confirms the pixels afterwards. This turned a
five-round guessing game with the owner into one unattended session.

**Fire TV smart-TV trap:** if the panel is ASLEEP (`dumpsys power` reports
`mWakefulness=Asleep`) Kodi hangs at startup right after the `GLES: Maximum
texture width` line, with no crash and no error. `adb shell input keyevent
KEYCODE_WAKEUP` first, always. Delete the probe add-on when finished.

## 2.5 Apple TV / tvOS (ATV1) - a different dev loop entirely

Everything in section 2 assumes ADB. The Apple TV has none. Established 2026-07-09
while shipping modv2plus 1.5.0, on ATV1 (Apple TV 4K, tvOS, Kodi 21.3,
...
- **`TakeScreenshot` HARD-CRASHES Kodi on tvOS.** Every form: no-arg, and
  ...
  `"OK"` and silently does nothing. **There is no way to screenshot Kodi on an
  Apple TV.** Verify visually by eye, or on a Fire TV where `adb screencap` works.
- **JSON-RPC cannot call builtins.** To run `ReloadSkin()` (or any builtin) push a
  tiny dev-only `xbmc.python.script` add-on whose `default.py` does
  `xbmc.executebuiltin(" ".join(sys.argv[1:]))`, restart Kodi once so it scans,
  `Addons.SetAddonEnabled`, then `Addons.ExecuteAddon(addonid, params=["ReloadSkin()"])`.
  Delete it afterwards with the self-uninstall pattern (`shutil.rmtree` on its own
  basename-guarded dir); devicectl has no remove.
- **JSON-RPC keeps answering while the GUI is frozen.** A `pong` proves nothing
  about whether the box is usable. Measure a GUI stall from kodi.log timestamps:
  the gap between `initialize new skin` and `Loading skin file: Home.xml`.
- **Killing Kodi is not a clean shutdown**, so any `Settings.SetSettingValue` or
  `Skin.SetBool` from that session is LOST. Same clobber class as everything else
  in `kodi-settings-clobber.md`.

## 2.6 Fonts are bound at window load. Do not build a font toggle.

`CGUIControlFactory::Create()` reads `<font>` once and resolves it to a
`CGUIFont*` by exact name. `$VAR`/`$INFO` substitution happens in `CGUIInfoLabel`,
which only ever touches label TEXT. Therefore:

- `<font>$VAR[MyFontVar]</font>` does NOT switch fonts. It looks up a font literally
  named `$VAR[MyFontVar]`, finds none, and falls back. Silently.
- A font can only change when its window (re)loads. `SettingsCategory.xml` is
  `KEEP_IN_MEMORY`: it parses on FIRST open and is not re-parsed on reopen. So even
  a load-time `<include condition>` needs a full `ReloadSkin()` to take effect.
- **`ReloadSkin()` from a control's click path hangs Kodi.** Measured twice on ATV1:
  41.3s of frozen GUI (kodi.log gap) as a raw `<onclick>` builtin, and again after
  hopping to a script thread via `RunScript`. Driven from JSON-RPC it is fine
  (~0.1s). Never put it on a settings control.
- **The clock toggle is not a counter-example.** `font_clock` is Roboto-Thin and
  never changes; `ClockLabelVar` wraps the LABEL in `[B]...[/B]`, and Kodi
  re-evaluates label variables every render. Bold is markup, not a font. That trick
  needs a `<label>` you own, which Kodi's engine-cloned "Default Category Button"
  (SettingsCategory, DialogAddonSettings) does not give you.

Conclusion, encoded in 1.5.0: ship one opinionated look. The side navs are
permanently thin; there is no toggle to hang the box.
...
### You are not the only thing that calls ReloadSkin()

`apply_home_menu()` clears script.skinshortcuts' cache and hash so the menu is rebuilt. When
skinshortcuts finishes that rebuild it calls **`ReloadSkin()` itself** (in its `xmlfunctions.py`) -
we never ask it to. So an apply reloads the skin **twice**: once from skinshortcuts, once from us.

A skin reload destroys every open dialog. On a box that had just been restored, EZ Maintenance++'s
post-restore prompts open at exactly the same moment the boot service applies the patch, and the
reload tore the device-name `select()` out from under the user. A destroyed `select()` returns
`-1`, which reads as "keep the current name", so the wizard silently advanced and the next prompt
appeared. What the owner saw: a dialog flashes past, then a different question. Log proof:

```text
12:27:25.541 [mod v2+] cleared skinshortcuts skin.estuary.modv2.hash
12:27:34.799 initialize new skin...          <- skinshortcuts reloading, 9s later
12:27:35.173 [mod v2+] skinshortcuts menu built
```

Deferring only our own reload does not fix it: the destructive reload is skinshortcuts'. The boot
service therefore defers the **entire apply** until the screen is idle - `gui_busy()` is
`System.HasActiveModalDialog` OR a post-restore onboarding in progress (EZM's
`.ezm_buffer_prompt` marker). Apply nothing while a dialog is up, and apply as soon as one isn't.

**The general rule:** before an operation that reloads the skin, prove nothing is on screen -
including dialogs owned by other add-ons, and including reloads triggered by code you call rather
than code you wrote. `reload_skin_when_idle()` is the only path to `ReloadSkin()` in this add-on.

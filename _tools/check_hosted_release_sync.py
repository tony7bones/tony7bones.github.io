...
proxy's ``addons/hosted/<id>/addon.xml`` version. A mirror under
``addons/hosted/`` may point its GitHub-Releases asset template at a SOURCE
repo we also own (today only ``script.ezmaintenanceplusplus`` ->
``moquette/kodi-ezmpp``; ``skin.estuary7`` -> ``moquette/kodi-estuary7`` was
the second until the skin's 2026-08-31 decommission unpublished it), and
